...
const paragraphs = [
	...
	"In the dense jungle, the air was thick with humidity. Vines hung from towering trees, their leaves a vibrant green. The calls of exotic birds echoed through the foliage, adding to the symphony of jungle sounds.",
...
app.get('/', (req, res) => {
    res.json({
        status: "success",
        message: "Random Paragraph API is running!",
        total_available_paragraphs: paragraphs.length,
        available_endpoints: [
            "/paragraph (Returns 1 random paragraph)",
            "/paragraph?count={number} (Returns multiple random paragraphs)"
        ]
    });
});
...
app.get("/paragraph", (req, res) => {
    try {
        ...
        const maxCount = Math.min(count, paragraphs.length);

        // Shuffle the array and select the top 'maxCount' elements
        const shuffled = [...paragraphs].sort(() => 0.5 - Math.random());
        const randomParagraphs = shuffled.slice(0, maxCount);

        res.json({
            count: randomParagraphs.length,
            total_available: paragraphs.length,
